...
def seed_database() -> None:
    ...
    settings = {
        ...
        "yandex_map_iframe": "https://yandex.ru/map-widget/v1/?text=%D0%A1%D0%B5%D1%80%D0%B3%D0%B8%D0%B5%D0%B2%20%D0%9F%D0%BE%D1%81%D0%B0%D0%B4%2C%20%D0%92%D0%BE%D1%80%D0%BE%D0%B1%D1%8C%D1%91%D0%B2%D1%81%D0%BA%D0%B0%D1%8F%2016%D0%90&z=17",
    ...
    db.session.add_all([
        ...
        Review(author_name="Екатерина С.", author_initials="ЕС", child_info="сын Артём, 6 лет", text="Ходим на\u00a0логопедию уже\u00a0год. За\u00a0это время Артём заговорил чище, научился слышать звуки и\u00a0не\u00a0боится отвечать у\u00a0доски. Ребёнок идёт к\u00a0педагогу с\u00a0удовольствием — а\u00a0для\u00a0нас это главное.", rating=5, program_id=programs[1].id),
        Review(author_name="Наталья К.", author_initials="НК", child_info="дочь Лиза, 2 года", text="Пришли на\u00a0раннее развитие по\u00a0рекомендации подруги. Педагоги — огонь: внимательные, тёплые, каждому ребёнку — свой подход. Дочка с\u00a0первого занятия спрашивает, когда снова пойдём в\u00a0Семицветик.", rating=5, program_id=programs[3].id),
        Review(author_name="Ирина Д.", author_initials="ИД", child_info="мама Саши, 5 лет", text="Искали место, где ребёнок будет развиваться без\u00a0давления. В\u00a0«Семицветике» нашли именно такое: занятия построены как\u00a0игра, но\u00a0результат — реальный. Сын читает и\u00a0считает, хотя ему ещё год до\u00a0школы.", rating=5, program_id=programs[0].id),
        Review(author_name="Ольга В.", author_initials="ОВ", child_info="двойняшки Кира и\u00a0Миша, 4 года", text="Два\u00a0ребёнка — два\u00a0характера. В\u00a0центре нашли подход к\u00a0каждому: одному — спокойные занятия, другому — активные. Удивительно, что\u00a0команда так\u00a0чутко чувствует детей.", rating=5, program_id=programs[3].id),
        Review(author_name="Мария Т.", author_initials="МТ", child_info="сын Костя, 7 лет", text="Водим на\u00a0английский второй\u00a0год. Педагог живой, играет, поёт, не\u00a0сидит по\u00a0учебнику — и\u00a0в\u00a0этом вся\u00a0магия. Ребёнок заговорил и\u00a0не\u00a0боится делать ошибок.", rating=5, program_id=programs[2].id if len(programs) > 2 else programs[0].id),
    ])
